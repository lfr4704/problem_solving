...
function getMaxProfit() {
    ...
    for (let earlierTime = 1; earlierTime < yesterdayStockPrices.length; earlierTime++) {
        //this first if statement is to test if the scenario of a stock that lost money all day. 
        if (yesterdayStockPrices[0] > yesterdayStockPrices[earlierTime] ) {
            
            maxProfit = yesterdayStockPrices[yesterdayStockPrices.length-2] - yesterdayStockPrices[yesterdayStockPrices.length-1]
            
        //the else statement is for the scenario where stock increased and decreased throught the day    
        } else {
